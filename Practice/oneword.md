...
#### 061
C
- それぞれ登場回数カウントしてキーで昇順ソートして、ループでvalueを加算してKを越えたら超える直前のキーが答え
...
E
- むりむりむりむりかたつむり
  - 解説意味不明だしググって出た解説みながらやっても無限に時間かかったんだが？？？？
  - 極上にぶたん
    - なんでこれが教育的なのか わたしに教育は向いてないというのか 死にたい 鬱になった 向いてないね 諦めてやめちまえば？
- にぶたんでl,rを指定する時、ちゃんと問題の条件内で「とりうることがない最大値」「とりうることのない最小値」を指定しないと駄目、っていうので初めて殺された問題
- あと`a[i]-b[i]//mid`ってめちゃくちゃむずいだろこれ気づかんわ
...
- 過去にACしてた
- 見た感じ通りのにぶたん
- ...が、なんか実装できず、仕方なく以前書いたPython3のACコードをGoで書き直す
- ...が！！！！！！！なんかしらんけどWA。そのまま移しただけのはずなのだが。意味不明
- 意味不明すぎて数十分無駄にする。他の人の通ってるコードをパクる。どうやらにぶたんの部分にコツがありそうだということはわかったもののいまだにわけがわからず。
- なんとなくコツっぽい？ところを以下に記す。しかしこれどういう場合にこれがいいのかわからん
```go
...
#### 152
D
- なんやこの簡単そうな顔していざ実装ができなくて死にかけて解説みたら超簡単という自分の凡百さをこれでもかと露呈してしまうフィルタリング問題は（怒）
- DPとかBFSとか使わなくていい ふつうに全探索して最初の文字と最後の文字を10*10程度の二重配列で保存して前後を入れ替えたものをかけたものを足し合わせて終了、は？
- なんでこれが解けない こんなもん秒で倒せ 向いてないからしかたないね つまらん
...
D
- 虚無、無情なほどに難しい
  - 単純な顔してガチガチにむずい式変形とか概念なんだけどなんでこれがこんなに解かれているんですか?
  - 自分以外がみな天才、馬鹿なのはお前だけ
- nCrと繰り返し二乗法と逆元と死と鬱

#### 157
D
- unionfindの良問（解けなかったけど）
  - **union findでそれぞれの木に属するノードの数を数える**をやるやり方を実践したのは初めてかもしれない。知見
- 図を書くと見えてくる（そう言われればそうのレベル）が、答えは、**今見てるノードが属する木に属するノードの総数 - (そこから直に生えてるノードの数 + 今見てるノードとブロック関係にあるやつ + 今見てるノード自身)**となる
- のでそうする、としたいところだが実装がわりと応用なので厳しかった
- 「直に生えてるノード」と「ブロック関係にあるノード」を出すためだけに隣接リストを作る
...
- 本番ぶりに解き直そうとしたら24時間くらいかかったよ 退化してるね
- 今回はGoで解いたのでLinkedListもdequeも使ってない 左右と反転回数を上手く使う
- `head, body, tail = [ab], [bd], [ce]`みたいなとき、**最終的に**反転回数が
  - 偶数ならば
    - 適当に0のときのことを思うといいんだけど「後ろに追加してく」方針なので、**headが逆順になる**。
  - 奇数ならば
    - ↑の逆、といいたいところだが、所与のSも逆になるので、**body,tailがぎゃくになる**。
- というのを気をつける
  - ってのを丁寧にGoでやったら1880msくらいだった ギリギリ
...
D
- 何度目かのアタック、簡単そうな見た目とdifのはずなのにまっっったく解けないと自分がいかに愚鈍で矮小で生きるに値しない存在なのかと思い知らされる
  - かわいそうだね
- nCrなんだけど都度やってると当然間に合わないのでどうするかなんだが、最初テーブルで登場回数を管理してそれぞれの値の`ncr(v,2)`と`ncr(v-1, 2)`をアレしようとしたらわけわからんくなったあぼーん
- 40分くらい悩んで、これは最大ベース（1つも取らない場合で同じものを取る組み合わせ）がすべての値で同じなので、こいつからそれぞれの値特有のなんかを引いてやればいけると天啓
- なんか、がなんなのかわからんかったので適当にn=5あたりまで`nCr(n,2)`を手でやると、登場回数-1が使えそうになったのでそうした
- ncr([n=2, 3, 4, 5, 6...], 2) -> 1, 3, 6, 10, 15...
- いやもうなんか苦手
...
- 落ち着いて考えれば`O(1)`
  - なんだけど本番中はキレながら5WAした
D
- 本番中は脳死で桁DPだと思い込み、そこから離れられず死亡
  - 桁DPは**今見ている桁を考える**のであって、**前回の桁の影響を考えるものではない**ということだ、多分
  - 終わった直後のTLに「DはBFS」とあってあっはい死にますねとなった
- ということでふつうにBFSする。わりとかなり愚直な感じでも余裕で通った
  - そのぶん死にたみが増すね

#### 161
本番3完, -30ポイントくらいで色落ちした....
C
- 落ち着いて問題文を読むとせいぜい1パス先なのでNについて全探索でいい

D
- 解けなかった 死亡
- 解説、だからなんでそうなるんだという感じ
- A,Bについて-120~120くらいまでを全探索するんだけどマジで理解できねぇよ
...
#### 006
B
- どうみてもやるだけの問題なんだけどgolangでやったら空白を扱うためにわざわざbufio.NewReaderを拵えたら中身が空になって受け取れてないことになってて、でもローカルではちゃんと受け取れてるのでこれはもうわけわかんなくて、調べてもわかんなかったので諦めて`sc.Split(ScanWords)`を消してぜんぶ一行ずつとってNとLはわざわざ`strconv.Atoi`してやり、残りも逐次`sc.Scan()`のち`sc.Text()`してやるっていう馬鹿みたいなムーブをするまで6時間くらいかけたので大馬鹿だと思う二度とこんな問題やりたくないつまらんくだらん何も面白くない何が面白いんだこれ？
#### 007
 A
- まだABCのC==ARCのAじゃなかった時代？
...
#### 017
B
- 10回以上アタックして今回1時間以上かけてACした...が.....
- 見た目からしてしゃくとりっぽいのでそうした。条件を帳尻合わせすることが（あたまがわるいので）まったくできず時間を溶かす。
- 死にそうになりながらACしたあと解説みたら実はしゃくとりはオーバーキルだということが判明
- 想定解は、`伸ばせるところまで伸ばした長さ-K+1`を加えていくというもの。なんで気づかなかったんだろう。向いてないね。つらいね。こんな気持になるくらいなら二度と競プロやらないほうがいいね。
...
#### 022
B
- みるからに尺取なんですがなんか永遠にバグらせてしまった
  - アホなので`A[l] == A[r]`としていた 最初に限らないよね...
  

#### 023
B
- サンプル3が超特大ヒント これがなかったら気づかなかった
- すなわち、Dの偶奇で行ける場所が分けられるのと、Dの大きさでいける範囲の上限が示される。
- 二次元テーブルを与えられるしてっきり探索系かと思いきや、ステップ数の偶奇という制約で見えてくるものがあるという気づき
...
#### 036
B
- しゃくとりだと脳死したが冷静になると累積和でいける
- 図を思い浮かべれば増加部分列と減少部分列ががっちんこした最大長だとわかる


#### 037
B
- これなんだよ超ムカつく めっちゃ時間無駄にした
  - 木の条件を思い出して解けたけどクソみてぇにいろいろ無駄なことを試した
...
B
- バチャコンで初見で解けたのでえらい
- たとえばa,b,cがそれぞれ一回ずつしか登場せず、d,eが2回ずつ登場するとき、d,eを[a,b,c]がそれぞれ買い分数になるように均等に割り振ればdad,ebe,cみたいになる
- ので、「1つしかないやつの左右に余ったのをくっつける、のをできるだけ均等にやる」となる。1つしかないやつは1以上の奇数なら1だけとって残りは余剰分に回すのと同じ
- 1つしかないやつがない、つまりぜんぶ偶数のときは長さNの回分数が構成可能
- 割り振るのは左右2つで1セットなので2で割った数を考える
- 均等に割り振ると最大でどれくらい割り振れるかを考えると1+(余剰分の総数を2で割ったもの/一つしか無いやつの総数)*2となる。+1は一つしか無いやつ自身

#### 054
A
- shit one
- 最悪に苦手なタイプ
  - 致命的にあたまがわるいのでこんなshitな問題なんかに1時間かける羽目になる
- 場当たり的にバグを潰していっただけなのでまともな方針はない
  - せいぜいx,yとs,dの比較で4パターンありそう、程度
    - ほんとは距離間を考えるともうちょい複雑になりそう、くらいの予測がつく程度
  - いやまじで小学校のとき距離と速さと時間の計算がまったくできなかったトラウマを思い出す
    - お前今もできてないじゃん
- 鬱になる
...
#### 068
D
- 93476837回アタックしてやっとだよ
- 理屈はしらんけど手を動かしたら、3枚以上あるやつはその偶奇で1枚か2枚にできる
  - 全部1か2にできる
- 2が偶数枚だと全部1にできる 奇数枚だと1枚減る
...
- 5ヶ月ぶりに解いた
  - 今回もすげぇ不安というかクソ勘でやったけどどうにか一発AC
  - 数と「ここに赤があり得るかどうか」のリストを別々にもっておいて適切に更新
  
#### 003
B
- 令和ABCだとC程度相当
  - 問題文の意味がわからんがそれはおいといてどうやらサンプルをみれば浮かぶ
  - 後ろのやつを優先して残さず召し上がったほうがいい
  - っていうただの貪欲
...
#### 008
B
- Nが非常に小さいのでなんか別の方法があるやもしれんのだがアホなのでにぶたんをした
  - そういう意味ではにぶたん入門かもしれない
- いくつならいいかをmidにして実際に試す。必要な最大のぶんに引っ張られるので更新する。要素Aがnameに10,kitに3なら4つ必要なのでceilする
- 解説がないので想定解がわからん（他の提出を読まないカス）
...
- 4ヶ月ぶりに解いた
  - 前回のロジックの混乱がほぼない なんで混乱してたん？
  - ncrでやるのもわかったし、「偶数を選ぶ全ての組み合わせ」*「奇数を選ぶ全ての組み合わせ」ってのも分かるっしょ
  - 

#### 018
A
- 何億回アタックしたかわからんわ
  - 通したけど10回ほど提出してWAのたびにifを追加していくクソムーブｷﾒた
- 条件分岐が無限に分からん
- 大雑把に、1が作れれば終了で、奇数と偶数があればいいんだけど、奇数だけでも偶数作れるからいっそ奇数だけあればいいんだけど、駄目な場合があって、それが無限に続く
- ああああああああああああああああ fuckin shit cocksuckkkkkkker
...
- 最強に難しい200点らしい
  - 解説見て散歩したら合点がいった
  - むしろ今の境地なら教育的良問とすら評価できるレベル
    - 散歩する前はガチギレで破壊衝動の鬼と化していたのはヒミツ
- 累積和応用
  - 累積和の特徴（包除原理みたいなやつ）を使いつつ、さらに一歩難しい
  - `A[i]...A[j]`の累積和を`B[j]-B[i-1]`で表すと題意より`B[j]-B[i-1] = 0`の箇所を数えることになる
  - よって`B[j] = B[i-1]`となるi,j(i<j)の組の組み合わせの総数となる
  - これは適当に2つ選ぶ組み合わせなのでその数がP個あるなら`P*(P-1)/2`でいける
  - これ累積和で最初に0を加えて長さN+1にするタイプの累積和だと気づかないんだけど、**2つ選ばなくてもそれ自体で0になる**場合のとき、**0を加えず長さNにした**累積和だと0の数がおかしくなる。
    - 後出しの理由しか思いつかん
      nCrができない、みたいな
    - 誰か分かるように説明できるやつおる？
    
- 類題で更に難しくしたやつがABC105D。

B
- サンプル眺めてたら「差が1のLIS」」をNから引けばいいと分かる
  - 演繹的に、途中に挿入することはできないのでそれはそう
- ただし実装がちょっと浮かばず雑にmapで飛び飛びでいけるとこまでいけばわかるんじゃね？と思い,相変わらず計算量は知らんがアルコールキメて寝て翌朝アタックしてなんとか実装
  - しゃくとりでやった。他にそのままズバリの差1のLISが候補として思いついたがやってない
- 想定解の実装が意味分からん。O(N)らしいがなんでこれで最長がとれるんだ？
...
- 二ヶ月に解き直した
  - TLE笑 成長ゼロ 記憶力なし
  - 辞書にして通したけどそれまでに3TLEしてましたからねアホちゃうか
...
A
- 10回以上アタックしてようやくAC
  - 前回はひと月前でGoで書いてたが無限にWAしてた
- とはいえ（多分）今までの方針は同じで
  - 最大でK分割になることは自明で、sが3文字以上ならば確実に2個以上に分割できる
  - 最初の1文字を`pre`としてスタートし、`s[i]`が前回と同じなら+1した2文字ぶんを`pre`とし、異なるならその文字を`pre`として答えをインクリメント、今が2文字ならば次は1文字分だけとればいいのでそうする、というのをやる
- 今回はPythonで書いた
...
#### JOI2009 yo
C
- 愚直再帰でやったらTLEになった これは実装ゲキムズでは
D
- 「おっ典型DFSか？」となるも、スタート地点を任意に決められるということで、それぞれ四方を探索したときに進むステップ数でどうやって無駄を省くかを考えてしまい、もちろんこれは愚考で死亡。解答を見る羽目になる
  - もうちょい詳しく書くと、「再長経路のはじっこをピックアップできたらそれらをふつうにDFSすりゃいいのでは？」とわけわからん方向に舵を切ってしまった。
- 解説によると **マス[i, j]をスタート地点にしたときの再長距離** だけを求めればいいということだ。そりゃそうで、制限的にも間に合うのだ
- 解説のコードを脳死でパクったら死んだのでいままでどおりのdfsを書いた
...
#### 全国統一プログラミング王決定戦予選（nikkei2019-qual)
C
- ゲキムズ 無理 無限に時間溶かした なんだこれ 不可能
  - n時間やって駄目 もうお前は駄目 無知無能無価値非存在
- 解説みてもわけがわからん なんやこの理屈
  - あたまがわるいんだね 向いてないからね
...
#### 第二回ドワンゴからの挑戦状 dwango2016-prelims
B
- ひと月前にやったときは6時間くらいハマった挙げ句結局とけなかったっぽい
- 問題は微妙に憶えてたけど、今回読んだらふつうに解けた
- 両端以外の真ん中は小さい方にひっぱられるので`min(a[i], a[i+1])`でいいし、両端はそのまんま先頭と末尾でいい
- なんでこれそんなにハマってたん？ 
...
#### CODE FESTIVAL 2014 Fianl
C
- 百億回アタックした
  - アタックのたびに問題の意味が分からず1byteも書けないを繰り返していた 
  - は？？？？？
- 今回はちゃんと書き出してみてどうなるを観察した ここまでは基本で今までもやっとる
  - 23 -> 23^1*2 + 23^0*3 なので一般化すると
  - x -> ... + (int(x[-2]+x[-1])^1*x[-1] + x[0]^0*x[0] 
- 入出力の意味がわからん Aって何？
- で、最後のサンプルをみたら答えの最大値が1000ぽいので全探索の気持ちになる
- つまり答えを決め打ちして合ってるかどうか確かめる問題になった
- しんどい,けど気づいたのはえらい
...
- 2ヶ月ぶりに解き直した
- 問題覚えてないけど全部足してから引いていく構造が問題文から浮かんだので実装
  - a-bがデカいのをインデックス保持しなきゃいけないんじゃね？とか勘違いしてたが必要ない情報
- ただし最初-1のケースを忘れて2WAして、修正したと思ったら謎のバグが発生して3提出連続WA
  - ○ね ○す
- 仕方ないのでクソムーブで解いた なんか知らんけど条件式で駄目ケースが排除できてないか良いケースも駄目にしてる
  - まるで意味がわからん
  - ○ぬ ○せ
...
B
- やるだけ
- 辞書使ってカウントするとのキーでのソートがめんどい言語だとめんどい。Mぶんの配列もつこともできるよ
C
...
B
- 最大のやつに引っ張られそうなので、最大のやつとそれ以外のやつ、として切り分ける
  - 最大のやつを1つずつ並べた間にそれ以外のやつをはさんでいく感じ
- すると、それ以外の総数が最大のやつ-1以上だったら最大のやつを使わなくてすむことになんとなく気づく
- すぐに解法のようなものがうっすらと浮かぶが、それを詰めるのが疑心暗鬼
...
#### Tenka1 Programmer Contest 2014 予選B
A
- 必ず一箇所だけ発現するのでその場所のインデックスを保持すりゃあとはそれまでのS[i]とそのあとのS[i+6]で`HAGIZILE`を挟む
- 一箇所の見つけ方は、今見てる`i`から`i+6`文字まで探索して調べても間に合うのでそうする
  - 尤も、文字列操作系の関数でincludeとかがあればそれが楽なんだがな
...
#### Tenka1 Programmer Beginner Contest 2018 天下一プログラマービギナーコンテスト
C
- これに何度アタックしたんだろう、たぶん10*n+m時間以上は吸われている気がする
  - あたまがわるいのでアタックのたびに解説みても理解不能で、問題文の雰囲気以外一切憶えてないんだろうね
- 今回もまたx時間くらい眺めて、たぶんp回目くらいの同じ方針が浮かんだ
  - **最弱が単調増加/減少なのでその逆をやる、すなわち今とれる最強（もしくは最弱）のあとは今取れる最弱（もしくは最強）を取る**
  - 両方試せばイイネ!
- これで過去x-1回ずっっっっっっとWAを出してた
- これで作られる配列をみてみるとたしかにサンプルと合わない。でもよくみると**末尾を先頭にもってくるとサンプルになる**と気づく。
  - これが最も謎 なんでこれが浮かんだか、なんでこれで合ってるのか完全に謎
- ひと月ぶりにやったらクソ駄目でした 一切覚えてない 記憶力がゼロ 生きてる価値無い
  - 強いやつを優先的にとったものとその先頭を最後に移したものの2つで比較して6WAだが「弱いやつを優先的に取る」をしてないせいか・？？？？？？？？？
  - ↑の内容だと強い優先/弱い優先に加えてそれぞれ先頭を末尾にくっつけるタイプの4つを用意しているってこと？？？？？？？？
  - じゃあ**なんで末尾を先頭に持っていくのは無いのか**誰か説明してくれるかこれ？？？？？？
- つーか↑ これ想定解じゃないじゃん 嘘
  
#### caddi 2018
C
- 素因数分解してそれぞれの冪数がNよりでかかったら採用できるだけ採用する
...
#### 第一回日本最強プログラマー学生選手権予選 jsc2019-qual
B
- 10**10+7回くらいアタックしてやっっっっっっとだよ畜生
  - 今回も3時間くらいかけたぞ畜生....
- さんすう
  - のつもり、具体的には等差数列の和として解いたんだけどどうも想定解と異なるっぽい
- A単独で考えたとき、左からみて`A[i]<A[j]`は愚直でわかる。右から見て`A[i]>A[j]`も数えると、左から見たやつは初項、左から見たやつと右からみたやつの和は公差という感じの数列としてみると解けた
...
B
- 2兆万回やった そのたびに解説みてたけど意味不明だったやつ
  - 以前のアタックで見た解説をうっすらと覚えていたので実装
  - めちゃくちゃ実装しにくかった気がするが終わってしまえばそれはそうとなって超ムカつく
...
#### m-solution プロコンオープン2019
D
- 解説で語られている理由の意味はまっっっっったく理解できないが、サンプルを眺めたら合計値から最大値を省いたものが答えになるっぽく、そのようにするには最大値を葉にしてあとは大きい順に隣接ノードを埋めていけばなんとかなりそうだったのでそうした
...
- 200点問題だけど実装がキレイにいったので記念
  ...
  - ```python
    ...
    ```
C
- 10^10^10くらいアタックした気がする
- たぶんいままでのコードは脳死でにぶたんをやっていた
  - 今回も最初はにぶたんでやってて無限に実装に至らずn時間を溶かしたあと、別の方法で実装して10分ほどだった
- こういう差分の問題は差分そのものでソートしてやる
  - **差分でソートして大きく足りないものは大きく余ってるやつをぶつける**
- 両端から攻めていくしゃくとり的な実装。足りなくなったら借りる、をやる。
...
- 部分和問題
  - 部分和って二次元DPなんだな
  - がんばってループで解けた。ナップザック的にやる

B
- は？わからんわ
  - 後手の場合に何を返せばいいのかわからず適当にminしたり引いたりしてしまった
  - **得点を与えない**っていうのが答えなんだけどなぜそうなるのかはすっと理解できん
  - これminmax法らしい
- メモ化再帰でやったんだけどループで解けない、アホか？
