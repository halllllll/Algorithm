...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if query == 0 {
			...
			x, y := nextInt(), nextInt()
			if same(x, y) {
				fmt.Fprintln(out, cost(x)-cost(y))
			} else {
				fmt.Fprintln(out, "?")
			}
		}
...
func cost(a int) int {
	...
	return weights[a] + cost(parents[a])
}

func union(a, b, w int) {
	root_a, root_b := root(a), root(b)
	if root_a != root_b {
		// ここから
		if b != root_b {
			weights[root_a] = w - cost(a)
		} else {
			weights[root_a] = w - (cost(a) - weights[root_b])
		}
		// ここまで
		parents[root_a] = b
	}
