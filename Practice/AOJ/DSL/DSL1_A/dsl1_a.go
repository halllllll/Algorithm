...
func unite(arr []int, a, b int) {
	if same(arr, a, b) {
		return
	}
	root_a, root_b := root(arr, a), root(arr, b)
	arr[root_a] = root_b
}

func root(arr []int, x int) int {
	if arr[x] == x {
		return x
	}
	y := root(arr, arr[x])
...
func same(arr []int, a, b int) bool {
	root_a, root_b := root(arr, a), root(arr, b)
...
func main() {
	...
	for i := 0; i < q; i++ {
		query, a, b := nextInt(), nextInt(), nextInt()
		if query == 0 {
			unite(arr, a, b)
		} else {
			if same(arr, a, b) {
